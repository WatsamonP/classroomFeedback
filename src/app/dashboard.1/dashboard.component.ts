  ...
  ngOnInit() {/*
    ...
    this.item.subscribe(item => {
      this.dataService.fetchData()
        .subscribe(data => {
          this.teachUid = Object.keys(data);
          for (var i = 0; i < this.teachUid.length; i++) {
            // ไม่เจอรายวิชา
            if (data[this.teachUid[i]].course == undefined || data[this.teachUid[i]].course == null) {
              continue;
            }
            // เจอแล้ว
            this.courseTemp = Object.keys(data[this.teachUid[i]].course);
            for (var j = 0; j < this.courseTemp.length; j++) {
              //console.log(data[this.userTemp[i]].course[this.courseTemp[j]].students)
              this.stdTemp = Object.keys(data[this.teachUid[i]].course[this.courseTemp[j]].students);
              for (var k = 0; k < this.stdTemp.length; k++) {
                // stdIdFetch = object ของ Student
                let stdIdFetch = data[this.teachUid[i]].course[this.courseTemp[j]].students[this.stdTemp[k]];
                ...
                if (String(item.stdId) == String(stdIdFetch.id)) {
                  ...
                  this.courseDate = Object.keys(data[this.teachUid[i]].course[this.courseTemp[j]].schedule.attendance);
                  this.studentCourse.push({
                      cId: data[this.teachUid[i]].course[this.courseTemp[j]].id,
                      cName: data[this.teachUid[i]].course[this.courseTemp[j]].name,
                      cGroup: stdIdFetch.group,
                      cTeacher: String(data[this.teachUid[i]].profile.firstName + ' ' +data[this.teachUid[i]].profile.lastName),
                      isClick: false,   //แสดงวันที่
                      isClick2: false,  //แสดง Rating
                      isClick3: false,  //แสดงกล่อง comment + ปุ่ม
                      attendance: data[this.teachUid[i]].course[this.courseTemp[j]].students[this.stdTemp[k]].attendance,
                      //[""0""].couseAttendance.schedule.attendance[""2018-08-09-08-02-26""]
                      //[1].couseAttendance.schedule.attendance[""2018-07-08-13-00-43""]
                      // 
                      couseAttendance: data[this.teachUid[i]].course[this.courseTemp[j]].schedule,
